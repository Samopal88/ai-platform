...
token = env.get("ANTHROPIC_AUTH_TOKEN", "") or env.get("CLAUDEHUB_API_KEY", "")
...
variants = [
    ("x_api_key", {"x-api-key": token, "anthropic-version": "2023-06-01", "content-type": "application/json"}),
    ("bearer", {"Authorization": f"Bearer {token}", "anthropic-version": "2023-06-01", "content-type": "application/json"}),
    ("both", {"x-api-key": token, "Authorization": f"Bearer {token}", "anthropic-version": "2023-06-01", "content-type": "application/json"}),
]

results = []
for name, headers in variants:
    try:
        response = httpx.post(f"{base}/v1/messages", headers=headers, json=payload, timeout=60)
        results.append({
            "variant": name,
        ...
        results.append({"variant": name, "ok": False, "error": f"{type(exc).__name__}: {exc}"})
